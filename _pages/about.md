---
layout: page
title: About
permalink: /about/
comments: false
custom_class: about-page
---

<div class="academic-cv">

  <header class="cv-hero">
    <div class="cv-photo-wrap">
      <img class="cv-photo" src="{{ '/assets/images/profile.jpg' | relative_url }}" alt="Eunhye Choi profile photo">
    </div>

    <div class="cv-intro">
      <h1 class="cv-name">Eunhye Choi</h1>
      <p class="cv-role">M.S. Student, KAIST School of Computing / Graduate School of Metaverse</p>

      <p>
        I am an M.S. student at <strong>KAIST</strong>, advised by
        <strong>Prof. Jinah Park</strong> in the Computer Graphics and Visualization Lab.
        My research interests include <strong>Generative AI</strong>, <strong>XR</strong>,
        <strong>Metaverse</strong>, computer graphics, and interactive systems.
      </p>

      <div class="cv-links">
        <a href="mailto:eunhye0323@kaist.ac.kr">Email</a>
        <span>·</span>
        <a href="https://www.linkedin.com/in/eunhye8348/" target="_blank" rel="noopener noreferrer">LinkedIn</a>
        <span>·</span>
        <a href="#publications">Publications</a>
        <span>·</span>
        <a href="#contact">Contact</a>
      </div>
    </div>
  </header>

  <section class="cv-section" id="about">
    <h2>About</h2>
    <p>
      I received my B.S. from Ajou University in Digital Media and Software &amp; Computer Engineering,
      and I am currently pursuing my M.S. at KAIST.
      My work spans generative AI, immersive media, virtual environments, and human-computer interaction.
    </p>
    <p>
      I aim to build research and systems that connect AI, graphics, and interaction in meaningful ways,
      especially for future digital content, XR, and intelligent visual computing.
    </p>
  </section>

  <section class="cv-section cv-two-col">
    <div>
      <h2>Education</h2>
      <ul>
        <li>
          <strong>Korea Advanced Institute of Science and Technology (KAIST)</strong><br>
          M.S. in School of Computing / Graduate School of Metaverse<br>
          <span class="cv-muted">September 2024 – Present</span>
        </li>
        <li>
          <strong>Ajou University</strong><br>
          B.S. in Digital Media / Software and Computer Engineering<br>
          <span class="cv-muted">March 2020 – August 2024</span>
        </li>
      </ul>
    </div>

    <div>
      <h2>Research Interests</h2>
      <ul>
        <li>Generative AI</li>
        <li>Extended Reality (XR)</li>
        <li>Metaverse</li>
        <li>Computer Graphics</li>
        <li>Interactive Systems</li>
      </ul>
    </div>
  </section>

  <section class="cv-section" id="experience">
    <h2>Work &amp; Research Experience</h2>
    <ul>
      <li>
        <strong>Researcher</strong>, Computer Graphics and Visualization Lab, KAIST
        <span class="cv-muted">March 2025 – Present</span>
      </li>
      <li>
        <strong>Researcher</strong>, Graduate School of Metaverse, Games and Life Lab, KAIST
        <span class="cv-muted">September 2024 – February 2025</span>
      </li>
      <li>
        <strong>Researcher</strong>, Interactive Entertainment Lab, Ajou University
        <span class="cv-muted">December 2022 – August 2024</span>
      </li>
    </ul>
  </section>

  <section class="cv-section" id="publications">
    <h2>Selected Publications</h2>

    <div class="pub-group">
      <h3>Conference</h3>
      <ol class="pub-list">
        <li>
          <strong>TouchArt: Vibrotactile Interaction for Color Perception in Virtual Painting</strong><br>
          Gyuna Lim, Anney Vo, <strong>Eunhye Choi</strong>, Jinah Park<br>
          <em>2025 IEEE World Haptics Conference, Hands-On Demonstrations</em>
        </li>
        <li>
          <strong>Press Start to Continue: A Thematic Analysis of the Iterative Process of Hardcore Players with Disabilities Adapting to Gameplay Difficulties</strong><br>
          Eunbyul Park*, Jihun Chae*, Karam Eum, <strong>Eunhye Choi</strong>, Hyunyoung Oh, Youngyim Doh<br>
          <em>2025 CHI Conference on Human Factors in Computing Systems, Late Breaking Work</em>
        </li>
        <li>
          <strong>RAG-Enhanced LLM Chatbot for Game Accessibility: Development and Evaluation of GAIA</strong><br>
          Hyunyoung Oh, Karam Eum*, Eunbyul Park*, Jihun Chae*, Junsuk Seo*, <strong>Eunhye Choi</strong>*, Youngyim Doh<br>
          <em>2025 HCI Korea</em>
        </li>
        <li>
          <strong>Prompting-Based LLM Framework for Ethical Decision-Making in the Trolley Dilemma: Embedding Hofstede’s Cultural Dimension Theory (PLETH)</strong><br>
          <strong>Eunhye Choi</strong>*, Jihun Chae*, Youngyim Doh<br>
          <em>2024 Fall Conference on Korean Artificial Intelligence Association</em>
        </li>
        <li>
          <strong>BCI Neurofeedback Game based on Attention and Meditation Training</strong><br>
          <strong>Eunhye Choi</strong>, Gyuhwan Oh<br>
          <em>2023 Spring Conference on Korea Game Society</em>
        </li>
      </ol>
    </div>

    <div class="pub-group">
      <h3>Journal</h3>
      <ol class="pub-list">
        <li>
          <strong>Analysis of the Aesthetic Elements of Art Game through the Player's Experience</strong><br>
          <strong>Eunhye Choi</strong>, Jiyeon Ahn<br>
          <em>Journal of Art Education, vol.72, February 2023, pp. 297–330.</em>
        </li>
      </ol>
    </div>

    <div class="pub-group">
      <h3>Book Chapter</h3>
      <ol class="pub-list">
        <li>
          <strong>Future Education and Art Education: Future, World, Digital, AI, Virtual Reality, Metaverse, NFT, Art, Education</strong><br>
          <strong>Eunhye Choi</strong>, Jiyeon Ahn et al.<br>
          <em>Education Science Publishing, January 2024, pp. 175–213.</em>
        </li>
      </ol>
    </div>
  </section>

  <section class="cv-section" id="awards">
    <h2>Awards</h2>
    <ul>
      <li><strong>2024</strong> — 3rd Place, Woongjin ThinkBig Award, Virtual Convergence Service Developer Competition</li>
      <li><strong>2023</strong> — 2nd Place, NIPA President's Award, XR Device Content Makeathon</li>
      <li><strong>2023</strong> — Excellent Paper Award, Korea Game Society Spring Conference</li>
      <li><strong>2022</strong> — 1st Place, Grand Award, Gyeonggi Culture Technology Academy</li>
      <li><strong>2022</strong> — 2nd Place, Excellence Award, Gyeonggi Metaverse Academy</li>
    </ul>
  </section>

  <section class="cv-section" id="teaching">
    <h2>Teaching Experience</h2>
    <ul>
      <li><strong>TA</strong>, Metaverse and Human Psychology, KAIST <span class="cv-muted">(Fall 2024)</span></li>
      <li><strong>TA</strong>, Game Engine Programming, Ajou University <span class="cv-muted">(Spring 2023)</span></li>
      <li><strong>TA</strong>, Secondary Information, Ajou University Science Education Institute for the Gifted <span class="cv-muted">(2023)</span></li>
      <li><strong>TA</strong>, Understanding Games, Ajou University <span class="cv-muted">(Fall 2022)</span></li>
      <li><strong>TA</strong>, Creative Media, Ajou University <span class="cv-muted">(Spring 2022)</span></li>
      <li><strong>TA</strong>, Graphic Design, Ajou University <span class="cv-muted">(Fall 2021)</span></li>
    </ul>
  </section>

  <section class="cv-section" id="skills">
    <h2>Skills</h2>
    <ul>
      <li><strong>Languages:</strong> Python, C#, C, C++, GLSL</li>
      <li><strong>Tools:</strong> Git, GitHub, OpenGL, OpenCV, PyTorch, TensorFlow</li>
      <li><strong>Engines:</strong> Unity, Unreal</li>
      <li><strong>Spoken Languages:</strong> Korean (Native), English (Fluent)</li>
    </ul>
  </section>

  <section class="cv-section" id="contact">
    <h2>Contact</h2>
    <p><strong>Email:</strong> <a href="mailto:eunhye0323@kaist.ac.kr">eunhye0323@kaist.ac.kr</a></p>
    <p><strong>LinkedIn:</strong> <a href="https://www.linkedin.com/in/eunhye8348/" target="_blank" rel="noopener noreferrer">linkedin.com/in/eunhye8348</a></p>
  </section>

  <div class="cv-updated">
    Updated: 2026.03.07
  </div>

</div>